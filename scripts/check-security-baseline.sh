...
for path in "${scan_paths[@]}"; do
  [[ -e "$path" ]] || continue
  if grep -RInE '(AKIA[0-9A-Z]{16}|BEGIN RSA PRIVATE KEY|BEGIN OPENSSH PRIVATE KEY|ghp_[A-Za-z0-9_]{30,}|xox[baprs]-[A-Za-z0-9-]{10,}|api[_-]?key\s*=\s*["'"''][^"'"'']+["'"'']|password\s*=\s*["'"''][^"'"'']+["'"''])' "$path"; then
    echo "Potential secret detected. Remove it or explicitly redesign the check." >&2
    exit 1
  fi
done
...
if grep -RInE 'NSAllowsArbitraryLoads\s*</key>\s*<true/>' . 2>/dev/null; then
